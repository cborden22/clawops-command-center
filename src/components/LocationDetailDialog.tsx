...
export function LocationDetailDialog({
  ...
  const printCommissionSummary = (summary: CommissionSummaryRecord) => {
    ...
    const safeName = sanitizeForHTML(location.name);
    const safeContactPerson = sanitizeForHTML(location.contactPerson);
    const safeNotes = sanitizeForHTML(summary.notes);
  ...
  const printAgreement = (agreement: LocationAgreementRecord) => {
    ...
    const safeProviderName = sanitizeForHTML(agreement.providerName);
    const safeProviderAddress = sanitizeForHTML(agreement.providerAddress);
    const safeProviderContact = sanitizeForHTML(agreement.providerContact);
    const safeLocationName = sanitizeForHTML(location.name);
    const safeLocationAddress = sanitizeForHTML(location.address);
    const safeLocationContact = sanitizeForHTML([location.contactPerson, location.contactPhone, location.contactEmail].filter(Boolean).join(" - "));
    const safePaymentMethod = sanitizeForHTML(agreement.paymentMethod);
    const safeNoticePeriod = sanitizeForHTML(agreement.noticePeriod);
      ...
      <body>
        ...
        <div class="parties">
          ...
          <div class="party-section">
            ...
            <div><strong>Name:</strong> ${safeLocationName || "[Business Name]"}</div>
            <div><strong>Address:</strong> ${safeLocationAddress || "[Business Address]"}</div>
            <div><strong>Contact:</strong> ${safeLocationContact || "[Contact Information]"}</div>
          </div>
        </div>
